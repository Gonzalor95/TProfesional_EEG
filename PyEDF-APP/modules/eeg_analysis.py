import eeg_analysis_utils as eeg_utils
import argparse
...
parser.add_argument('-f', '--filter', choices=["lowpass", "highpass", "SMA"], default=None)
...
input_signal = eeg_utils.generate_input_signal(args.measured_signal, args.test_signal, args.channels, resample=True, filter=filters[args.filter] if args.filter else None)
output_signal = eeg_utils.generate_output_signal(args.measured_signal, args.channel, resample=False, filter=filters[args.filter] if args.filter else None)
