...
from TestingSignals import TestingSignalsWorker
...
class Filter(ABC):
    ...
    def configure_filter(self, data=None, fs=None, order=None, cutoff=None, window_size=None):
        """
...
def plot_signal_comparison(s1, s2, title, label_1 = None, label_2 = None, input_lag=0):
    ...
    plt.plot(output_time_axis - input_lag, s2, 'b--', label="Output signal" if not label_2 else label_2)
    plt.grid(True)
    plt.title(title)
    plt.legend()
    plt.show()
...
def get_correlation_offset_for_window(input_signal, output_signal):
    ...
    calculate_mse_for_signal_window(input_signal, windowed_output_signal, input_lag, SIGNAL_WINDOW_SIZE)
    return input_lag
...
def generate_input_signal(input_signal: str, is_testing: bool, channels: list, t_signal_type= "Sinusoidal", t_freq=1, t_amp=199, t_sr=500, t_dur=5, filter: Filter=None, resample=False):
    curr_script_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    ...
    input_signal = input_signal_worker.signal_data_.physical_signal if is_testing else input_signal_worker.signal_data_.physical_signals_and_channels[0][1]
    if filter:
        filter.configure_filter(data=input_signal)
        input_signal = filter.apply_filter()
    if resample:
        input_signal = resampy.resample(input_signal, input_signal_worker.getSampleRate(), 200)
    return input_signal

def generate_output_signal(signal_filepath, channels, filter: Filter=None, resample=False):
    ...
    if resample:
        output_signal = resampy.resample(output_signal, output_signal_worker.getSampleRate(), 200)
